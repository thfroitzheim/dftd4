...
pure subroutine get_2b_damp(self, param, r2, rdamp, d6, d8)
   ...
   real(wp) :: r1, rdamp_exp, arg, erf_val, reff, inv, inv2, inv6

   r1 = sqrt(r2)

   ! Extract the original radius: rdamp = a1 * r0 + a2
   rdamp_exp = (rdamp - param%a2) / param%a1

   arg = -param%a3 * (r1 - param%a4 * rdamp_exp)
...
pure subroutine get_2b_derivs(self, param, r2, rdamp, d6, d8, d6dr, d8dr)
   ...
   real(wp) :: r1, rdamp_exp, arg, erf_val, reff, inv, inv2, inv6,  dreff
   ...
   dreff = -rdamp * param%a3 * exp(-arg * arg) / sqrtpi
...
pure subroutine get_3b_damp(self, param, r, r2ij, r2ik, r2jk, &
   ...
   real(wp) :: rij, rik, rjk, rdamp_exp_ij, rdamp_exp_ik, rdamp_exp_jk
   ...
   ! Extract the original radius: rdamp = a1 * r0 + a2
   rdamp_exp_ij = (rdampij - param%a2) / param%a1
   rdamp_exp_ik = (rdampik - param%a2) / param%a1
   rdamp_exp_jk = (rdampjk - param%a2) / param%a1

   arg_ij = -param%a3 * (rij - param%a4 * rdamp_exp_ij)
   arg_ik = -param%a3 * (rik - param%a4 * rdamp_exp_ik)
   arg_jk = -param%a3 * (rjk - param%a4 * rdamp_exp_jk)

   reff_ij = rdampij * 0.5_wp * (1.0_wp + erf(arg_ij))
   reff_ik = rdampik * 0.5_wp * (1.0_wp + erf(arg_ik))
   reff_jk = rdampjk * 0.5_wp * (1.0_wp + erf(arg_jk))
...
pure subroutine get_3b_derivs(self, param, r, r2ij, r2ik, r2jk, &
   ...
   dreff_ij = -rdampij * param%a3 * exp(-arg_ij * arg_ij) / sqrtpi
   dreff_ik = -rdampik * param%a3 * exp(-arg_ik * arg_ik) / sqrtpi
   dreff_jk = -rdampjk * param%a3 * exp(-arg_jk * arg_jk) / sqrtpi
