...
pure subroutine get_2b_damp(self, param, r2, rdamp, d6, d8)
   ...
   real(wp) :: r1, r6, rdamp2, rdamp6, rdamp_exp, ef, sf, sig

   r1 = sqrt(r2)
   r6 = r2 * r2 * r2

   ! In CSO the rational damping radii are replaced by only the constant a2
   rdamp2 = param%a2 * param%a2
   rdamp6 = rdamp2 * rdamp2 * rdamp2

   ! Extract the original radius assuming: rdamp = r0 + a2
   ! a1 is assumed to be 1 (no direct radius scaling)
   rdamp_exp = (rdamp - param%a2) / param%a1

   ef = exp(r1 - param%a4 * rdamp_exp)
   sf = 1.0_wp / (1.0_wp + ef)
...
pure subroutine get_2b_derivs(self, param, r2, rdamp, d6, d8, d6dr, d8dr)
   ...
   real(wp) :: r1, r4, r6, rdamp2, rdamp6, rdamp_exp, ef, sf, sig, t6, dsig
   ...
   ! Extract the original radius assuming: rdamp = a1 * r0 + a2
   ! a1 is assumed to be 1 (no direct radius scaling)
   rdamp_exp = (rdamp - param%a2) / param%a1
